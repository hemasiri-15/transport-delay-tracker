...
calculate_all_delays() {
    ...
    log_message "INFO" "Delay calculation complete. Processed $record_count records"
...
EOF
    ...
    log_message "INFO" "Statistics calculated: $on_time_pct% on-time, avg delay $avg_delay min"
...
main() {
    ...
    if [[ -z "$delay_file" ]] || [[ ! -f "$delay_file" ]]; then
